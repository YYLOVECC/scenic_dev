/**
 * Created by licong on 2017/2/15.
 */
...
var valid_form = function () {
    var name = $('#adminusersform-name').val();
    var email = $('#adminusersform-email').val();
    var post_name = $.trim(name);
    var post_email = $.trim(email);
    for (var i = 0; i<GLOBAL.user_info.length; i++) {
        if (name == GLOBAL.user_info[i]['name']) {
            layer.alert('名称不能相同');
            return false;
        }
        if (email == GLOBAL.user_info[i]['email']) {
            layer.alert('邮箱不能相同');
            return false;
        }
    }
